#!/bin/bash
# Example usage: ./test_authorizations.sh -l -r <path to dir containing skydentity repo> -k <path to public key file> -s <path to corresponding secret key> -c <path to key file of service account w/ firestore access> -y <yaml-dir> -p <project id>
...
while getopts "r:c:k:s:y:p:l" option; do
  case $option in
    r)
