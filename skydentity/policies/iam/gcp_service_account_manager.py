...
from skydentity.policies.checker.gcp_authorization_policy import GCPAuthorizationPolicy, Authorization, RestrictedRole
#import httplib2
...
class GCPServiceAccountManager:
    ...
    def __init__(self, credentials_path: str) -> None:
        ...
        self._cloudresourcemanager_service = googleapiclient.discovery.build("cloudresourcemanager", "v3", credentials=self._credentials)

    def create_service_account(self, authorization: GCPAuthorizationPolicy, service_account_name):
        ...
        if service_account_name not in self._service_accounts:
            ...
            try:
                ...
                self._service_accounts[service_account_name] = (
                    self._iam_service.projects().serviceAccounts().create(
    ...
    def add_roles_to_service_account(self, authorization: GCPAuthorizationPolicy, service_account_name):
        ...
        if service_account_name in self._service_accounts:
            ...
            try:
                ...
                raise ValueError(f"Service account {service_account_name} does not exist in project {auth.project}")
        service_email = service_account["email"]

        # Get current policy to modify and add roles
        print(f"Project: {auth.project}")
        ...
        for new_binding in auth.roles:
            iam_policy["bindings"].append({
                ...
                "members": [
                    f"serviceAccount:{service_email}"
                ],
                "condition": {
                    "title": "skydentity",
