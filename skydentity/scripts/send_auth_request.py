...
parser.add_argument(
    "--resource_yaml_input", type=str, help="The path to the input resource yaml file"
)
parser.add_argument(
    "--resource_yaml_output",
    type=str,
    help="The path to write the output resource yaml file",
)
parser.add_argument(
    "--auth_request_yaml", type=str, help="The path to the auth request yaml file"
)
parser.add_argument(
    "--capability_enc_key",
    type=str,
    help="The path to the capability encryption key file",
)
parser.add_argument(
    "--capability_enc_out",
    type=str,
    help="The path to the resulting encrypted capability JSON file",
)
...
def check_capability(
    capability: dict, capability_enc_path: str
    ...
    header = b64decode(
        capability["header"]
    )  # If checking broker public key, check that the header matches the public key attached to the request
...
def main():
    ...
    if args.cloud == "gcp":
        ...
        raise ValueError(f"Invalid cloud {args.clodu}")
    response = send_auth_creation_request(
        f"{protocol}://127.0.0.1:{port}/skydentity/cloud/{args.cloud}/",
        json=auth_request_dict,
    )
    print("RESPONSE JSON", response.json())
    service_acct_id, valid_capability = check_capability(
        response.json(), args.capability_enc_key
    )
    ...
    with open(args.resource_yaml_input, "r") as f:
        ...
        print(resource_dict)
        if args.cloud == "gcp":
            resource_dict["attached_authorizations"][0]["gcp"][0][
                "authorization"
            ] = service_acct_id
        elif args.cloud == "azure":
            resource_dict["virtual_machine"]["attached_authorizations"][0][args.cloud][
                0
            ]["authorization"] = [service_acct_id]
        with open(args.resource_yaml_output, "w") as f:
            yaml.dump(resource_dict, f)
    with open(args.capability_enc_out, "w") as f:
        print("Output capability at:", args.capability_enc_out)
