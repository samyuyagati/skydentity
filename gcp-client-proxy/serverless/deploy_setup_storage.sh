...
if $CREATE_SECRETS; then
  gcloud secrets create sa-key-storageproxy --data-file=$SERVICE_ACCT_KEY_PATH
  openssl rand 32 > ../local_tokens/capability_enc.key
  gcloud secrets create enc-key-storageproxy --data-file=../local_tokens/capability_enc.key
  ...
  gcloud projects add-iam-policy-binding $GCP_PROJECT \
    ...
    gcloud secrets versions add enc-key-storageproxy --data-file=../local_tokens/capability_enc.key
fi
...
gcloud builds submit --tag gcr.io/$GCP_PROJECT/skyidproxy-storage
